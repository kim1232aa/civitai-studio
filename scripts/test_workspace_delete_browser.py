...
with sync_playwright() as p:
    ...
    try:
        ...
        before = page.evaluate('JSON.parse(localStorage.getItem("nl-storyboard-v0821o7"))')
        page.once('dialog', lambda dialog: dialog.dismiss())
        button.click()
        assert page.evaluate('JSON.parse(localStorage.getItem("nl-storyboard-v0821o7"))') == before
        checks.append('planner cancel preserves full state')
        page.once('dialog', lambda dialog: dialog.accept())
        button.click()
        expect(button).to_have_count(0)
        page.reload(wait_until='domcontentloaded')
        saved = page.evaluate('JSON.parse(localStorage.getItem("nl-storyboard-v0821o7"))')
