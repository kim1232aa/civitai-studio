...
class FalContract(unittest.TestCase):
    ...
    def test_krea_turbo_lora_sample_keeps_endpoint_and_sends_loras(self):
        ...
        self.transport.reset_mock()
        with self.assertRaises(ValueError):
            fal.build_fal_input(null_scale)
        ...
        self.transport.reset_mock()
        with self.assertRaises(ValueError) as raised:
            fal.build_fal_input(missing_scale)
        miss = str(raised.exception).lower()
        self.assertTrue("scale" in miss or "strength" in miss)
        self.assertTrue("1.0" in str(raised.exception) or "缺省" in str(raised.exception))
