...
check("缺的字段点名 prompt", "prompt" in (d.get("errors") or [{}])[0].get("fields", []), str(d.get("errors")))

code, d = fp.whatif({"serviceId": "fal-ai/nano-banana-2/edit", "prompt": "make it night"})
expect("i2i 没接图打回", code, d, 400, "missing_input_media")

code, d = fp.whatif({"serviceId": "fal-ai/kling-video/v2.5-turbo/pro/image-to-video", "prompt": "pan"})
check("i2v 没接图打回", code == 400 and d.get("code") in ("missing_required", "missing_input_media"), str(d.get("error")))

code, d = fp.whatif({"serviceId": "fal-ai/flux/schnell", "prompt": "a cat", "width": 1024, "height": 1024})
expect("t2i 齐全放行", code, d, 200)
check("放行带 operation", d.get("operation") == "text-to-image", str(d.get("operation")))
check("放行带 submittedInput", (d.get("submittedInput") or {}).get("prompt") == "a cat", str(d.get("submittedInput")))
check("放行带 cost 说明", "没有预估接口" in ((d.get("cost") or {}).get("note") or ""), str(d.get("cost")))

code, d = fp.whatif({"serviceId": "fal-ai/nano-banana-2/edit", "prompt": "night", "images": ["https://x/a.jpg"]})
expect("i2i 接了图放行", code, d, 200)
check("i2i 落到 image_urls", "image_urls" in (d.get("submittedInput") or {}), str(d.get("submittedInput")))

code, d = fp.whatif({"serviceId": "fal-ai/flux/schnell", "prompt": "a cat", "images": ["https://x/a.jpg"]})
warn = [w["code"] for w in (d.get("warnings") or [])]
check("t2i 收到图要警告丢弃", code == 200 and "input_media_dropped" in warn, str(warn))

code, d = fp.whatif({"serviceId": "fal-ai/some-unlisted-endpoint/v9", "prompt": "a cat"})
warn = [w["code"] for w in (d.get("warnings") or [])]
check("目录无字段表要如实说明", code == 200 and "no_schema" in warn, str(warn))

code, d = fp.whatif({"serviceId": "fal-ai/esrgan", "prompt": "upscale", "images": ["https://x/a.jpg"]})
...
passed_through = [x["id"] for x in sweep if fp.whatif({"serviceId": x["id"], "prompt": "a cat"})[0] != 400]
check(
...
killed = [x["id"] for x in t2i if fp.whatif({"serviceId": x["id"], "prompt": "a cat"})[0] != 200]
...
check("目录错标端点以路径为准判成吃图", all(
    fp.whatif({"serviceId": i, "prompt": "a cat"})[1].get("code") == "missing_input_media" for i in mislabeled
), str(mislabeled))
...
if hf is None:
    hf = next(
        obj() for name, obj in vars(hf_mod).items()
        if isinstance(obj, type) and name.endswith("Provider") and obj is not hf_mod.Provider
    )

HUB_T2I = {"pipeline_tag": "text-to-image", "inferenceProviderMapping": {"fal-ai": {"status": "live", "providerId": "fal-ai/flux/schnell"}}}
HUB_I2I = {"pipeline_tag": "image-to-image", "inferenceProviderMapping": {"nscale": {"status": "live", "providerId": "some/edit-model"}}}
...
code, d = hf.whatif({"serviceId": "black-forest-labs/FLUX.1-schnell", "prompt": "a cat"})
expect("t2i 齐全放行", code, d, 200)
check("放行报出实际供应商", (d.get("service") or {}).get("provider") == "fal-ai", str(d.get("service")))

hf_mod.hub_probe = lambda mid: (200, HUB_I2I)
code, d = hf.whatif({"serviceId": "some/edit-model", "prompt": "night"})
expect("i2i 没接图打回", code, d, 400, "missing_input_media")

code, d = hf.whatif({"serviceId": "some/edit-model", "prompt": "night", "images": ["https://x/a.jpg"]})
expect("i2i 路由到不带图的通道要打回", code, d, 400, "input_media_dropped")

hf_mod.hub_probe = lambda mid: (0, {"error": "network down"})
code, d = hf.whatif({"serviceId": "black-forest-labs/FLUX.1-schnell", "prompt": "a cat"})
warn = [w["code"] for w in (d.get("warnings") or [])]
check("Hub 连不上要如实说没校验", code == 200 and d.get("verified") is False and "hub_unreachable" in warn, f"{code} {warn} {d.get('verified')}")
...
code, d = ms.whatif({"serviceId": "Qwen/Qwen-Image", "model": "Other/Model", "prompt": "x"})
...
code, d = ms.whatif({"serviceId": "MusePublic/Qwen-Image-Edit", "prompt": "night", "images": ["https://x/a.jpg"]})
expect("编辑模型接了图放行", code, d, 200)
check("图落到 image_url", (d.get("submittedInput") or {}).get("image_url") == ["https://x/a.jpg"], str(d.get("submittedInput")))

code, d = ms.whatif({"serviceId": "Qwen/Qwen-Image", "prompt": "a cat", "width": 1024, "height": 1024})
expect("t2i 齐全放行", code, d, 200)
check("body 与 generate 同源", (d.get("submittedInput") or {}).get("size") == "1024x1024", str(d.get("submittedInput")))
check("放行带 endpoint", "images/generations" in ((d.get("checked") or {}).get("endpoint") or ""), str(d.get("checked")))
...
for mod, name in ((fal_mod, "fal"), (hf_mod, "huggingface"), (ms_mod, "modelscope"), (nano_mod, "nanogpt")):
    ...
    check(f"{name} whatif 不是常量 return", "return 200, {" not in src_txt.replace("\n", ""), name)
    check(f"{name} whatif 有拒绝分支", src_txt.count("400") >= 3, name)

print("PASS whatif-contract" if not fails else f"FAIL whatif-contract {len(fails)}: {fails}")
