...
OP_SPEC = {
    ...
    # text→video; prompt required (wire-only like t2i); no image input ever
...
def _unsupported_params(params: dict, caps: dict, op: str, node_id: str) -> dict | None:
    ...
    if op in ("i2v", "t2v"):
        if "duration" in params and not caps.get("videoDuration"):
            return _err(
                f"节点 {node_id}({op}) 的 duration 不在当前模型/provider 能力内",
                blocked=True,
                nodeId=node_id,
                parameter="duration",
            )
        if "aspectRatio" in params and not caps.get("videoAspect"):
            return _err(
                f"节点 {node_id}({op}) 的 aspectRatio 不在当前模型/provider 能力内",
                blocked=True,
                nodeId=node_id,
...
def compile_graph(graph: dict | None) -> dict:
    ...
    for nid in order:
        ...
        if op == "t2v":
            ...
            if not payload["serviceId"]:
                return _err(f"节点 {nid} 缺少 serviceId", nodeId=nid)
            # wire-only for seed / negative / loras (same rules as i2v)
            if "seed" in inputs:
                payload["seed"] = inputs["seed"]
            if "seed" in params and "seed" not in inputs:
                return _err(
            ...
            if "cfgScale" not in payload and "cfg" in payload:
                payload["cfgScale"] = payload["cfg"]
            wired_loras = inputs.get("loras")
            merged = _merge_loras(wired_loras)
            if "loras" in params and "loras" not in inputs:
                return _err(
                    f"节点 {nid} 的 LoRA 只认连线，禁止 params.loras 旁路",
                    blocked=True,
            ...
            if video_wired_on:
                return _err(
                    f"POC 暂不支持把 t2v {nid} 的 video 再接到下游；请只留一个视频汇点",
                    blocked=True,
                    nodeId=nid,
                )
            stage = {"id": nid, "op": "t2v", "payload": deepcopy(payload), "produces": "video"}
            stages.append(stage)
            sink_wiring[nid] = {"t2v": True, "inbound": {}, "out": {}}
            candidate_sinks.append((nid, payload))
            continue
