...
适配器：`providers/fal.py`。capabilities：`lora=path`，`loraPath=http`，`progress=queue`，`cancel=True`，`estimate=pricing_api`，`resolution=free_wh`，`negative=True`，`i2i=first_frame`，`i2v=fal_endpoint`，`maxRefs=9`（单图端点 catalog 收紧到 1），`refImagesField=image_urls`。
...
| `duration` | `duration` | 否 | kling→字符串 |
| `quantity`/`qty` | `num_images` | 否 | 1–12 |
...
| | |
| --- | --- |
| 形态 | 优先 `loras:[{path,scale}]`；或 `lora_url`/`lora_path`/`lora` + scale |
...
## 官方对照（2026-09-08，queue 文档）

来源：https://fal.ai/docs/documentation/model-apis/inference/queue

- 推荐生产用 **submit → poll/webhook**，不是同步 `run()`。
- 生命周期：`IN_QUEUE`（含 `queue_position`）→ `IN_PROGRESS`（可带 `logs`）→ `COMPLETED`（再 GET result；失败时有 `error` / `error_type`）。
- REST：`POST https://queue.fal.run/{endpoint}`，`Authorization: Key $FAL_KEY`；status `…/requests/{id}/status?logs=1`；result `…/requests/{id}`；cancel `PUT …/cancel`。
- 图结果常见 `images[].url`；**视频**常为 `video` 对象（Studio pickUrl 已吃 bare `video_url`）。
- 可选：`webhook_url`、`start_timeout`（`X-Fal-Request-Timeout`）、`X-Fal-No-Retry`、priority。
- Studio 对齐点：空 `prompt` 仍保留 key；`/out` 必须 materialize；长 i2v 勿用过短 poll。
